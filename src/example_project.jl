...
function pure_pursuit(v1, v2, v3, segments; ls = 2.0, max_vel = 10.0, timestamp = 0.2) #ls = lookahead time
    ...
    # while true
        ...
        alpha = 0.5
        while collision_constraint(predict_x1, predict_x2, v1.size, v2.size)<0 && u[1]+v>-0.5
            println("reduce")
            u[1] -= alpha
            predict_x1 = constant_velocity_prediction(center_x1, timestamp, u)
        end
        while collision_constraint(predict_x1, predict_x3, v1.size, v3.size)<0 && u[1]+v>-0.5
            u[1] -= alpha
            predict_x1 = constant_velocity_prediction(center_x1, timestamp, u)
        end
        if u[1] + v > max_vel
